...
pub fn run(input_file: &str) -> i64 {
    let numbers = std::fs::read_to_string(input_file).unwrap()
        .lines().map(|line| line.parse::<i64>().unwrap() * 811589153).collect_vec();
    ...
    for _ in 0..10 {
        for (num_idx, num_delta) in numbers.iter().enumerate() {
            ...
            let mut abs_delta = *num_delta % (numbers.len() as i64 - 1);
            if abs_delta < 0 {
                abs_delta += numbers.len() as i64 - 1;
            }
